...
@ActiveProfiles("test")
class AgentCommentControllerTest {
    ...
    void edit_returns_updated_envelope() throws Exception {
        AgentVersionComment c = new AgentVersionComment(99L, "alice", "v2");
        ...
        mockMvc.perform(patch("/api/web/agent-comments/123")
    ...
    void delete_returns_204() throws Exception {
        mockMvc.perform(delete("/api/web/agent-comments/123")
                        .with(authentication(aliceAuth()))
                        .with(csrf()))
    ...
    void pin_returns_updated_envelope() throws Exception {
        ...
        mockMvc.perform(post("/api/web/agent-comments/123/pin")
                        .with(authentication(aliceAuth()))
                        .with(csrf())
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"pinned\": true}"))
